...
class TaskListController: UITableViewController {
    ...
    // коллекция задач и их сортировка по наблюдателю
    ...
    func setTasks(_ tasksCollection: [TaskProtocol]) {
        ...
        // используем только те задачи, для которых определена секция
    ...
    // указываем текстовый заголовок для секции на основании ее индекса
    ...
    private func getConfiguredTaskCell_stack(for indexPath: IndexPath) -> UITableViewCell {
        ...
        // изменяем текст в ячейке
        cell.tytle.text = currentTask.title
        // изменяем символ в ячейке
        cell.symbol.text = getSymbolForTask(with: currentTask.status)
        
        // изменяем цвет текста
        if currentTask.status == .planned {
            cell.tytle.textColor = .black
            cell.symbol.textColor = .black
        } else {
            cell.tytle.textColor = .lightGray
    ...
    override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        // создаем действие для изменения статуса на "Запланирована"
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        // получаем данные о задаче для ее удаления
        let taskType = sectionsTypesPosition[indexPath.section]
        // удаляем задачу
    ...
    override func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        ...
        // удаляем задачу с места, с которого она перенесена
